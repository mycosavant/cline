...
describe("Extension Tests", function () {
	...
	it("should activate extension successfully", async () => {
		// Get the extension
		const extension = vscode.extensions.getExtension("onemind.klaus-dev")
	...
	it("should open sidebar view", async () => {
		...
		await vscode.commands.executeCommand("Klaus.plusButtonClicked")
	...
	it("should handle basic commands", async () => {
		...
		await vscode.commands.executeCommand("Klaus.historyButtonClicked")
		// Success if no error thrown
	})

	it("should handle advanced settings configuration", async () => {
		// Test browser session setting
		await vscode.workspace.getConfiguration().update("Klaus.disableBrowserTool", true, true)
		const updatedConfig = vscode.workspace.getConfiguration("Klaus")
		expect(updatedConfig.get("disableBrowserTool")).to.be.true

		// Reset settings
		await vscode.workspace.getConfiguration().update("Klaus.disableBrowserTool", undefined, true)
	})
})

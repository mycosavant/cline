...
'use server';
/**
 * @fileOverview Service for interacting with the OpenRouter API.
 */

import type { Message } from '@/components/chat/chat-interface'; // Assuming Message type is defined here

/**
 * Represents the configuration needed to communicate with OpenRouter.
 */
export interface OpenRouterConfig {
  /**
   * The API key for accessing OpenRouter.
   */
  apiKey: string;
  /**
   * The base URL for the OpenRouter API. Defaults to 'https://openrouter.ai/api/v1'.
   */
  baseUrl?: string;
   /**
   * The model to use for chat completion. Defaults to NEXT_PUBLIC_OPENROUTER_CHAT_MODEL env var or 'openai/gpt-4o-mini'.
   */
  model?: string;
}

/**
 * Represents a response from the OpenRouter API, containing generated content.
 */
 ...
 */
export async function generateChatCompletion(messages: Message[], config: OpenRouterConfig): Promise<OpenRouterResponse> {

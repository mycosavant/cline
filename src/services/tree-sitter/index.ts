...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = KlausIgnoreController ? KlausIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, KlausIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (KlausIgnoreController && !KlausIgnoreController.validateAccess(filePath)) {
		return null
	}

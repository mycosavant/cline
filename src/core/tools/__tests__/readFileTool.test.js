...
describe("read_file tool with maxReadFileLine setting", () => {
    ...
    async function executeReadFileTool(params = {}, options = {}) {
        ...
        await readFileTool(mockCline, toolUse, mockCline.ask, jest.fn(), (result) => {
            toolResult = result;
        }, (_, content) => content ?? "");
    ...
    describe("when maxReadFileLine is 0", () => {
        it("should return an empty content with source code definitions", async () => {
            ...
            expect(mockedParseSourceCodeDefinitionsForFile).toHaveBeenCalledWith(absoluteFilePath, mockCline.rooIgnoreController);
...
describe("read_file tool XML output structure", () => {
    ...
    describe("Basic XML Structure Tests", () => {
        ...
        it("should follow the correct XML structure format", async () => {
            ...
            const xmlStructureRegex = new RegExp(`^<file><path>${testFilePath}</path>\\n<content lines="1-5">\\n.*</content>\\n</file>$`, "s");
    ...
    describe("Line Range Tests", () => {
        it("should include lines attribute when start_line is specified", async () => {
            // Setup
            const startLine = 2;
            mockedReadLines.mockResolvedValue(fileContent
                .split("\n")
                .slice(startLine - 1)
                .join("\n"));
            // Execute
    ...
    describe("Notice and Definition Tags Tests", () => {
        ...
        it("should include list_code_definition_names tag when source code definitions are available", async () => {
            ...
            expect(result).toMatch(new RegExp(`<list_code_definition_names>[\\s\\S]*${sourceCodeDef.trim()}[\\s\\S]*</list_code_definition_names>`));
        });
    ...
    describe("Edge Cases Tests", () => {
        ...
        it("should handle file read errors correctly", async () => {
            ...
            // Verify
            expect(result).toContain(`<file><path>${testFilePath}</path><error>Error reading file: ${errorMessage}</error></file>`);
            expect(result).not.toContain(`<content`);
        });
    });
});

...
export function getCheckpointService(cline) {
    ...
    try {
        ...
        service.on("initialize", () => {
            ...
            try {
                const isCheckpointNeeded = typeof cline.clineMessages.find(({ say }) => say === "checkpoint_saved") === "undefined";
        ...
        service.initShadowGit().catch((err) => {
            log(`[Cline#getCheckpointService] caught unexpected error in initShadowGit, disabling checkpoints (${err.message})`);
            console.error(err);
            cline.enableCheckpoints = false;
        });
        return service;
...
export async function checkpointRestore(cline, { ts, commitHash, mode }) {
    ...
    try {
        ...
        if (mode === "restore") {
            ...
            const { totalTokensIn, totalTokensOut, totalCacheWrites, totalCacheReads, totalCost } = getApiMetrics(cline.combineMessages(deletedMessages));
            await cline.overwriteClineMessages(cline.clineMessages.slice(0, index + 1));
            // TODO: Verify that this is working as expected.
            await cline.say("api_req_deleted", JSON.stringify({

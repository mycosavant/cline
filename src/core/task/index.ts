...
import { KlausAskResponse, KlausCheckpointRestore } from "../../shared/WebviewMessage"
...
export class Task {
	...
	private KlausIgnoreController: KlausIgnoreController
	private askResponse?: KlausAskResponse
	...
	private async addToKlausMessages(message: KlausMessage) {
	...
	private async overwriteKlausMessages(newMessages: KlausMessage[]) {
		this.KlausMessages = newMessages
		await this.saveKlausMessagesAndUpdateHistory()
	}

	private async saveKlausMessagesAndUpdateHistory() {
		try {
			...
			const lastRelevantMessage =
				this.KlausMessages[
					findLastIndex(this.KlausMessages, (m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"))
				]
	...
	async restoreCheckpoint(messageTs: number, restoreType: KlausCheckpointRestore) {
		...
		try {
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.KlausMessages.slice(0, messageIndex),
				...
				const firstCheckpointMessageCheckpointHash = this.KlausMessages.find(
	...
	async handleWebviewAskResponse(askResponse: KlausAskResponse, text?: string, images?: string[]) {
		this.askResponse = askResponse
		this.askResponseText = text
		this.askResponseImages = images
	}

	async say(type: KlausSay, text?: string, images?: string[], partial?: boolean): Promise<undefined> {
		if (this.abort) {
			throw new Error("Klaus instance aborted")
		}
	...
	async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: KlausAsk | KlausSay) {
		const lastMessage = this.KlausMessages.at(-1)
	...
	private async resumeTaskFromHistory() {
		...
		if (lastApiReqStartedIndex !== -1) {
			...
			const { cost, cancelReason }: KlausApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const disableBrowserTool = vscode.workspace.getConfiguration("Klaus").get<boolean>("disableBrowserTool") ?? false
		...
		const preferredLanguage = getLanguageKey(
			vscode.workspace.getConfiguration("Klaus").get<LanguageDisplay>("preferredLanguage"),
		)
		...
		let KlausRulesFileInstructions: string | undefined
		...
		let KlausIgnoreInstructions: string | undefined
	...
	async presentAssistantMessage() {
		...
		switch (block.type) {
			...
			case "tool_use":
				...
				const askApproval = async (type: KlausAsk, partialMessage?: string) => {
				...
				switch (block.name) {
						...
						try {
							...
							if (diff) {
								...
								try {
									...
									const errorType =
										error instanceof Error && error.message.includes("does not precicely match.")
											? "search_not_found"
											: "other_diff_error"
							...
							const sharedMessageProps: KlausSayTool = {
							...
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Klaus wants to ${fileExists ? "edit" : "create"} ${path.basename(relPath)}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Klaus wants to read ${path.basename(absolutePath)}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Klaus wants to view directory ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Klaus wants to view source code definitions in ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Klaus wants to search files in ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
							...
							if (block.partial) {
								if (action === "launch") {
									...
									await this.say(
										...
										} satisfies KlausSayBrowserAction),
										undefined,
										block.partial,
									)
								}
								...
								if (action === "launch") {
									...
									if (this.shouldAutoApproveTool(block.name)) {
										...
										showNotificationForApprovalIfAutoApprovalEnabled(
											`Klaus wants to use a browser and launch ${url}`,
										)
										this.removeLastPartialMessageIfExistsWithType("say", "browser_action_launch")
						...
						try {
							if (block.partial) {
								...
								if (ignoredFileAttemptedToAccess) {
									...
									pushToolResult(
										formatResponse.toolError(formatResponse.KlausIgnoreError(ignoredFileAttemptedToAccess)),
									)
								...
								if (!requiresApproval && this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Klaus wants to execute a command: ${command}`,
									)
					...
					case "use_mcp_tool": {
						...
						try {
							if (block.partial) {
								...
								} satisfies KlausAskUseMcpServer)
								...
								if (mcp_arguments) {
									try {
										...
										await this.say(
											"error",
											`Klaus tried to use ${tool_name} with an invalid JSON argument. Retrying...`,
										)
										pushToolResult(
											formatResponse.toolError(
								...
								if (this.shouldAutoApproveTool(block.name) && isToolAutoApproved) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Klaus wants to use ${tool_name} on ${server_name}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Klaus wants to access ${uri} on ${server_name}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
									const didApprove = await askApproval("use_mcp_server", completeMessage)
									if (!didApprove) {
					...
					case "plan_mode_respond": {
						...
						const sharedMessage = {
							...
							options: parsePartialArrayString(removeClosingTag("options", optionsRaw)),
						} satisfies KlausPlanModeResponse
						try {
							if (block.partial) {
	...
	async recursivelyMakeKlausRequests(userContent: UserContent, includeFileDetails: boolean = false): Promise<boolean> {
		if (this.abort) {
			throw new Error("Klaus instance aborted")
		}
		...
		if (
			...
			await this.ask(
				...
				`Klaus has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`,
			)
			...
			try {
				this.checkpointTracker = await pTimeout(
					CheckpointTracker.create(this.taskId, this.controllerRef.deref()?.context.globalStorageUri.fsPath),
					{
						milliseconds: 15_000,
						message:
							"Checkpoints taking too long to initialize. Consider re-opening Klaus in a project that uses git, or disabling checkpoints.",
					},
				)
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : "Unknown error"
		...
		try {
			...
			const updateApiReqMsg = (cancelReason?: KlausApiReqCancelReason, streamingFailedMessage?: string) => {
				this.KlausMessages[lastApiReqIndex].text = JSON.stringify({
			...
			const abortStream = async (cancelReason: KlausApiReqCancelReason, streamingFailedMessage?: string) => {
				if (this.diffViewProvider.isEditing) {

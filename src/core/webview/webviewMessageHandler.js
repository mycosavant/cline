    ...
    switch (message.type) {
        ...
        case "resetSupportPrompt":
            try {
                ...
                provider.log(`Error reset support prompt: ${JSON.stringify(error, Object.getOwnPropertyNames(error), 2)}`);
        ...
        case "updatePrompt":
            ...
            break;
        case "deleteMessage": {
            const answer = await vscode.window.showInformationMessage(t("common:confirmation.delete_message"), { modal: true }, t("common:confirmation.just_this_message"), t("common:confirmation.this_and_subsequent"));
                        ...
                        if (apiConversationHistoryIndex !== -1) {
                            if (nextUserMessage && nextUserMessage.ts) {
                                ...
                                await provider
                                    ...
                                    ...provider
                                        .getCurrentCline()
                                        .apiConversationHistory.filter((msg) => msg.ts && msg.ts >= nextUserMessage.ts),
                                ]);
                            }
                            else {
                                ...
                                await provider
                                    .getCurrentCline()
                                    .overwriteApiConversationHistory(provider
                                    .getCurrentCline()
                                    .apiConversationHistory.slice(0, apiConversationHistoryIndex));
                            }
                        }
        ...
        case "enhancePrompt":
            if (message.text) {
                try {
                    const { apiConfiguration, customSupportPrompts, listApiConfigMeta, enhancementApiConfigId } = await provider.getState();

 ...
 */
export function convertToolUse(tool) {
    return {
        type: "tool_use",
        name: tool.name,
        params: tool.params,
 ...
 */
export function formatGroupedToolResults(mode, results, originalTools) {
    // Map results by toolId for easier reference
    const resultMap = new Map(results.map(r => [r.toolId, r.result]));

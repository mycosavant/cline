...
import { KlausAccountService } from "../../services/account/KlausAccountService";
import { McpHub } from "../../services/mcp/McpHub";
import { telemetryService } from "../../services/telemetry/TelemetryService";
import * as KlausController from './KlausController';
import { fileExistsAtPath } from "../../utils/fs";
import { searchCommits } from "../../utils/git";
import { getTotalTasksSize } from "../../utils/storage";
import { Task } from "../task";
import { openMention } from "../mentions";
import { getAllExtensionState, getGlobalState, getSecret, resetExtensionState, storeSecret, updateApiConfiguration, updateGlobalState, } from "../storage/state";
...
export class Controller {
    ...
    mcpHub;
    accountService;
    latestAnnouncementId = "march-22-2025"; // update to some unique identifier when we add a new announcement
    webviewProviderRef;
    constructor(context, outputChannel, webviewProvider) {
        this.context = context;
        this.outputChannel = outputChannel;
        this.outputChannel.appendLine("KlausProvider instantiated");
        this.webviewProviderRef = new WeakRef(webviewProvider);
        this.workspaceTracker = new WorkspaceTracker(this);
        this.mcpHub = new McpHub(this);
        this.accountService = new KlausAccountService(this);
    ...
    */
    async dispose() {
        this.outputChannel.appendLine("Disposing KlausProvider...");
        ...
        KlausController.KlausController?.dispose();
        this.mcpHub?.dispose();
    ...
    async handleSignOut() {
        try {
            await storeSecret(this.context, "KlausApiKey", undefined);
            await updateGlobalState(this.context, "userInfo", undefined);
            await updateGlobalState(this.context, "apiProvider", "openrouter");
            await this.postStateToWebview();
            vscode.window.showInformationMessage("Successfully logged out of Klaus");
        }
        catch (error) {
    ...
    async initKlausWithTask(task, images) {
        ...
        this.task = new Task(this, apiConfiguration, autoApprovalSettings, browserSettings, chatSettings, customInstructions, task, images);
    }
    async initKlausWithHistoryItem(historyItem) {
        await this.clearTask();
        const { apiConfiguration, customInstructions, autoApprovalSettings, browserSettings, chatSettings } = await getAllExtensionState(this.context);
        this.task = new Task(this, apiConfiguration, autoApprovalSettings, browserSettings, chatSettings, customInstructions, undefined, undefined, historyItem);
    }
    ...
    async handleWebviewMessage(message) {
        switch (message.type) {
            ...
            case "newTask":
                ...
                // initializing new instance of Klaus will make sure that any agentically running promises in old instance don't affect our new task. this essentially creates a fresh slate for the new task
                await this.initKlausWithTask(message.text, message.images);
                break;
            ...
            case "checkpointRestore": {
                ...
                // cancel task waits for any open editor to be reverted and starts a new Klaus instance
                if (message.number) {
                    // wait for messages to be loaded
                    await pWaitFor(() => this.task?.isInitialized === true, {
                        timeout: 3_000,
                    }).catch(() => {
                        console.error("Failed to init new Klaus instance");
                    });
                    // NOTE: cancelTask awaits abortTask, which awaits diffViewProvider.revertChanges, which reverts any edited files, allowing us to reset to a checkpoint rather than running into a state where the revertChanges function is called alongside or after the checkpoint reset
                    await this.task?.restoreCheckpoint(message.number, message.text);
                }
                break;
            }
            ...
            case "accountLoginClicked": {
                ...
                // 	`https://app.Klaus.bot/auth?state=${encodeURIComponent(nonce)}&callback_url=${encodeURIComponent(`${uriScheme || "vscode"}://onemind.klaus-dev/auth`)}`,
                // )
            ...
            case "downloadMcp": {
                if (message.mcpId) {
                    ...
                    const mcpConfig = vscode.workspace.getConfiguration("Klaus.mcp");
            ...
            // 	if (message.text) {
            // 		const response = await fetch(`https://api.Klaus.bot/v1/mcp/marketplace/item?mcpId=${message.mcpId}`)
            ...
            case "openExtensionSettings": {
                ...
                await vscode.commands.executeCommand("workbench.action.openSettings", `@ext:onemind.klaus-dev ${settingsFilter}`.trim());
                break;
            }
            case "invoke": {
                if (message.text) {
    ...
    async togglePlanActModeWithChatSettings(chatSettings, chatContent) {
        ...
        const { apiConfiguration, previousModeApiProvider: newApiProvider, previousModeModelId: newModelId, previousModeModelInfo: newModelInfo, previousModeVsCodeLmModelSelector: newVsCodeLmModelSelector, previousModeThinkingBudgetTokens: newThinkingBudgetTokens, planActSeparateModelsSetting, } = await getAllExtensionState(this.context);
    ...
    async cancelTask() {
        if (this.task) {
            ...
            if (this.task) {
                // 'abandoned' will prevent this Klaus instance from affecting future Klaus instance gui. this may happen if its hanging on a streaming request
                this.task.abandoned = true;
            }
            await this.initKlausWithHistoryItem(historyItem); // clears task again, so we need to abortTask manually above
            // await this.postStateToWebview() // new Klaus instance will post state when it's ready. having this here sent an empty messages array to webview leading to virtuoso having to reload the entire list
        }
    }
    ...
    async ensureMcpServersDirectoryExists() {
        ...
        const mcpServersDir = path.join(userDocumentsPath, "Klaus", "MCP");
        try {
            await fs.mkdir(mcpServersDir, { recursive: true });
        }
        catch (error) {
            return "~/Documents/Klaus/MCP"; // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
        }
    ...
    async handleAuthCallback(customToken, apiKey) {
        try {
            ...
            await storeSecret(this.context, "KlausApiKey", apiKey);
            // Send custom token to webview for Firebase auth
            await this.postMessageToWebview({
                type: "authCallback",
                customToken,
            });
            const KlausProvider = "Klaus";
            await updateGlobalState(this.context, "apiProvider", KlausProvider);
            // Update API configuration with the new provider and API key
            const { apiConfiguration } = await getAllExtensionState(this.context);
            const updatedConfig = {
                ...apiConfiguration,
                apiProvider: KlausProvider,
                KlausApiKey: apiKey,
            };
            if (this.task) {
            ...
            await this.postStateToWebview();
            // vscode.window.showInformationMessage("Successfully logged in to Klaus")
        }
        catch (error) {
            console.error("Failed to handle auth callback:", error);
            vscode.window.showErrorMessage("Failed to log in to Klaus");
    ...
    async fetchMcpMarketplaceFromApi(silent = false) {
        try {
            const response = await axios.get("https://api.Klaus.bot/v1/mcp/marketplace", {
                headers: {
    ...
    async downloadMcp(mcpId) {
        try {
            ...
            const response = await axios.post("https://api.Klaus.bot/v1/mcp/download", { mcpId }, {
...
- Use "${mcpDetails.mcpId}" as the server name in Klaus_mcp_settings.json.
...
Here is the project's README to help you get started:\n\n${mcpDetails.readmeContent}\n${mcpDetails.llmsInstallationContent}`;
            ...
            await this.initKlausWithTask(task);
            await this.postMessageToWebview({
                type: "action",
    ...
    // 'Add to Klaus' context menu in editor and code action
    async addSelectedCodeToChat(code, filePath, languageId, diagnostics) {
        // Ensure the sidebar view is visible
        await vscode.commands.executeCommand("klaus-dev.SidebarProvider.focus");
    ...
    // 'Add to Klaus' context menu in Terminal
    ...
    // 'Fix with Klaus' in code actions
    async fixWithKlaus(code, filePath, languageId, diagnostics) {
        // Ensure the sidebar view is visible
        await vscode.commands.executeCommand("klaus-dev.SidebarProvider.focus");
        await setTimeoutPromise(100);
        const fileMention = this.getFileMentionFromPath(filePath);
        const problemsString = this.convertDiagnosticsToProblemsString(diagnostics);
        await this.initKlausWithTask(`Fix the following code in ${fileMention}\n\`\`\`\n${code}\n\`\`\`\n\nProblems:\n${problemsString}`);
        console.log("fixWithKlaus", code, filePath, languageId, diagnostics, problemsString);
    }
    ...
    async showTaskWithId(id) {
        if (id !== this.task?.taskId) {
            ...
            await this.initKlausWithHistoryItem(historyItem); // clears existing task
        }
        await this.postMessageToWebview({
            type: "action",
            action: "chatButtonClicked",
    ...
    async getStateToPostToWebview() {
        const { apiConfiguration, lastShownAnnouncementId, customInstructions, taskHistory, autoApprovalSettings, browserSettings, chatSettings, userInfo, mcpMarketplaceEnabled, telemetrySetting, planActSeparateModelsSetting, } = await getAllExtensionState(this.context);
        return {
            ...
            KlausMessages: this.task?.KlausMessages || [],
    ...
    /*
    Now that we use retainContextWhenHidden, we don't have to store a cache of Klaus messages in the user's state, but we could to reduce memory footprint in long conversations.

    - We have to be careful of what state is shared between KlausProvider instances since there could be multiple instances of the extension running at once. For example when we cached Klaus messages using the same key, two instances of the extension could end up using the same key and overwriting each other's messages.
    - Some state does need to be shared between the instances, i.e. the API key--however there doesn't seem to be a good way to notify the other instances that the API key has changed.

    We need to use a unique identifier for each KlausProvider instance's message cache since we could be running several instances of the extension outside of just the sidebar i.e. in editor panels.
...
export * as KlausController from './KlausController';

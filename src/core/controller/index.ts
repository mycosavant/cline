...
import { KlausCheckpointRestore, WebviewMessage } from "../../shared/WebviewMessage"
...
export class Controller {
	...
	mcpHub?: McpHub
	accountService?: KlausAccountService
	...
	*/
	...
	async initKlausWithTask(task?: string, images?: string[]) {
	...
	async initKlausWithHistoryItem(historyItem: HistoryItem) {
		await this.clearTask()
		const { apiConfiguration, customInstructions, autoApprovalSettings, browserSettings, chatSettings } =
			await getAllExtensionState(this.context)
		this.task = new Task(
		...
		switch (message.type) {
			...
			case "checkpointRestore": {
				...
				if (message.number) {
					...
					await this.task?.restoreCheckpoint(message.number, message.text! as KlausCheckpointRestore)
				}
				break
			}
			...
			case "openExtensionSettings": {
				...
				await vscode.commands.executeCommand(
					...
					`@ext:onemind.klaus-dev ${settingsFilter}`.trim(), // trim whitespace if no settings filter
				)
				break
			}
			case "invoke": {
	...
	async handleAuthCallback(customToken: string, apiKey: string) {
		try {
			...
			const KlausProvider: ApiProvider = "Klaus"
	...
	private async downloadMcp(mcpId: string) {
		try {
			...
			const response = await axios.post<McpDownloadResponse>(
				"https://api.Klaus.bot/v1/mcp/download",
				{ mcpId },
				{
...
Here is the project's README to help you get started:\n\n${mcpDetails.readmeContent}\n${mcpDetails.llmsInstallationContent}`
	...
	async fixWithKlaus(code: string, filePath: string, languageId: string, diagnostics: vscode.Diagnostic[]) {
		// Ensure the sidebar view is visible
		await vscode.commands.executeCommand("klaus-dev.SidebarProvider.focus")
		await setTimeoutPromise(100)

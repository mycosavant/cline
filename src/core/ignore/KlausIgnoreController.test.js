import { KlausIgnoreController } from "./KlausIgnoreController";
import fs from "fs/promises";
import path from "path";
import os from "os";
import { after, beforeEach, describe, it } from "mocha";
import "should";
describe("KlausIgnoreController", () => {
    let tempDir;
    let controller;
    beforeEach(async () => {
        ...
        // Create default .Klausignore file
        await fs.writeFile(path.join(tempDir, ".Klausignore"), [".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join("\n"));
        controller = new KlausIgnoreController(tempDir);
    ...
    describe("Default Patterns", () => {
        ...
        it("should block access to .Klausignore file", async () => {
            const result = controller.validateAccess(".Klausignore");
    ...
    describe("Custom Patterns", () => {
        ...
        it("should handle pattern edge cases", async () => {
            await fs.writeFile(path.join(tempDir, ".Klausignore"), ["*.secret", "private/", "*.tmp", "data-*.json", "temp/*"].join("\n"));
            controller = new KlausIgnoreController(tempDir);
            await controller.initialize();
            const results = [
        ...
        // })
        it("should handle comments in .Klausignore", async () => {
            // Create a new .Klausignore with comments
            await fs.writeFile(path.join(tempDir, ".Klausignore"), ["# Comment line", "*.secret", "private/", "temp.*"].join("\n"));
            controller = new KlausIgnoreController(tempDir);
            await controller.initialize();
    ...
    describe("Error Handling", () => {
        ...
        it("should handle missing .Klausignore gracefully", async () => {
            // Create a new controller in a directory without .Klausignore
            const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`);
            await fs.mkdir(emptyDir);
            try {
                const controller = new KlausIgnoreController(emptyDir);
                await controller.initialize();
        ...
        it("should handle empty .Klausignore", async () => {
            await fs.writeFile(path.join(tempDir, ".Klausignore"), "");
            controller = new KlausIgnoreController(tempDir);
            await controller.initialize();
...
//# sourceMappingURL=KlausIgnoreController.test.js.map
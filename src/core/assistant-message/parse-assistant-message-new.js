 ...
 */
function parseToolCallsFromBlock(blockContent, execMode = "single") {
    const toolCalls = [];
    let pos = 0;
    while (pos < blockContent.length) {
        ...
        if (!toolUseNames.includes(tagName)) {
            pos = openTagEnd + 1;
            continue;
        }
        const toolName = tagName;
        // Find closing tag
 ...
 */
function processExecutionBlock(assistantMessage, blockType, tagName) {
    const contentBlocks = [];
    ...
    const mode = blockType === 'parallel' ? 'parallel' : 'sequential';
...
//# sourceMappingURL=parse-assistant-message-new.js.map
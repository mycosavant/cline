...
export class Klaus extends EventEmitter {
    taskId;
    instanceId;
    rootTask = undefined;
    ...
    // terminal
    terminalProcess;
    constructor({ provider, apiConfiguration, customInstructions, enableDiff = false, enableCheckpoints = true, fuzzyMatchThreshold = 1.0, consecutiveMistakeLimit = 3, task, images, historyItem, startTask = true, rootTask, parentTask, taskNumber = -1, onCreated, }) {
        super();
    ...
    static create(options) {
        const instance = new Klaus({ ...options, startTask: false });
    ...
    async ask(type, text, partial, progressStatus) {
        // If this Klaus instance was aborted by the provider, then the only
        // thing keeping us alive is a promise still running in the background,
        // in which case we don't want to send its result to the webview as it
        // is attached to a new instance of Klaus now. So we can safely ignore
        // the result of any active promises, and this class will be
        // deallocated. (Although we set Klaus = undefined in provider, that
        // simply removes the reference to this instance, but the instance is
        // still alive until this promise resolves or rejects.)
        if (this.abort) {
            throw new Error(`[Klaus#ask] task ${this.taskId}.${this.instanceId} aborted`);
        }
        let askTs;
        if (partial !== undefined) {
            ...
            if (partial) {
                if (isUpdatingPreviousPartial) {
                    ...
                    this.updateKlausMessage(lastMessage);
                    ...
                    await this.addToKlausMessages({ ts: askTs, type: "ask", ask: type, text, partial });
                ...
                if (isUpdatingPreviousPartial) {
                    ...
                    askTs = Date.now();
                    this.lastMessageTs = askTs;
                    await this.addToKlausMessages({ ts: askTs, type: "ask", ask: type, text });
                }
            }
        }
        else {
    ...
    async say(type, text, images, partial, checkpoint, progressStatus) {
        if (this.abort) {
            throw new Error(`[Klaus#say] task ${this.taskId}.${this.instanceId} aborted`);
        }
        if (partial !== undefined) {
            const lastMessage = this.clineMessages.at(-1);
            ...
            if (partial) {
                if (isUpdatingPreviousPartial) {
                    ...
                    await this.addToKlausMessages({ ts: sayTs, type: "say", say: type, text, images, partial });
                }
            }
            else {
                ...
                if (isUpdatingPreviousPartial) {
                    ...
                    const sayTs = Date.now();
                    this.lastMessageTs = sayTs;
                    await this.addToKlausMessages({ ts: sayTs, type: "say", say: type, text, images });
                }
            }
        }
        else {
            ...
            const sayTs = Date.now();
            this.lastMessageTs = sayTs;
            await this.addToKlausMessages({ ts: sayTs, type: "say", say: type, text, images, checkpoint });
        }
    }
    ...
    async startTask(task, images) {
        ...
        // if the extension process were killed, then on restart the clineMessages might not be empty, so we need to set it to [] when we create a new Klaus client (otherwise webview would show stale messages from previous session)
        this.clineMessages = [];
    ...
    async resumePausedTask(lastMessage) {
        // release this Klaus instance from paused state
        this.isPaused = false;
    ...
    async resumeTaskFromHistory() {
        const modifiedKlausMessages = await this.getSavedKlausMessages();
        ...
        this.clineMessages = await this.getSavedKlausMessages();
        ...
        const lastKlausMessage = this.clineMessages
            .slice()
            .reverse()
        ...
        const lastTaskResumptionIndex = newUserContent.findIndex((x) => x.type === "text" && x.text.startsWith("[TASK RESUMPTION]"));
    ...
    async initiateTaskLoop(userContent) {
        ...
        while (!this.abort) {
            ...
            // requests, but Klaus is prompted to finish the task as efficiently
            // as he can.
            if (didEndLoop) {
    ...
    async recursivelyMakeKlausRequests(userContent, includeFileDetails = false) {
        if (this.abort) {
            throw new Error(`[Klaus#recursivelyMakeKlausRequests] task ${this.taskId}.${this.instanceId} aborted`);
        }
        ...
        // In this Klaus request loop, we need to check if this task instance
        ...
        try {
            ...
            try {
                ...
                // Klaus instance to finish aborting (error is thrown here when
                ...
                if (!this.abandoned) {
                    ...
                    if (history) {
                        await provider?.initKlausWithHistoryItem(history.historyItem);
                    }
                }
            }
            finally {
    ...
    async *attemptApiRequest(previousApiReqIndex, retryAttempt = 0) {
        let mcpHub;
        const { apiConfiguration, mcpEnabled, autoApprovalEnabled, alwaysApproveResubmit, requestDelaySeconds } = (await this.providerRef.deref()?.getState()) ?? {};
        ...
        const { browserViewportSize, mode, customModePrompts, experiments, enableMcpServerCreation, browserToolEnabled, language, } = (await this.providerRef.deref()?.getState()) ?? {};
        ...
        const systemPrompt = await (async () => {
            ...
            return SYSTEM_PROMPT(provider.context, this.cwd, (this.api.getModel().info.supportsComputerUse ?? false) && (browserToolEnabled ?? true), mcpHub, this.diffStrategy, browserViewportSize, mode, customModePrompts, customModes, this.customInstructions, this.diffEnabled, experiments, enableMcpServerCreation, language, rooIgnoreInstructions);
        })();
    ...
    async presentAssistantMessage() {
        if (this.abort) {
            throw new Error(`[Klaus#presentAssistantMessage] task ${this.taskId}.${this.instanceId} aborted`);
        }
        ...
        switch (block.type) {
            ...
            case "tool_use": {
                // Check if this is a grouped tool execution (parallel or sequential)
                if (block.mode === "parallel" || block.mode === "sequential") {
                    // Import dynamically to avoid circular dependencies
                    const { executeToolsInParallel, executeToolsSequentially } = await import("./tool-execution");
                    const { formatGroupedToolResults } = await import("./tool-execution/tool-helpers");
                    if (this.didRejectTool) {
                        // Skip execution if a tool was already rejected
                        this.userMessageContent.push({
                            type: "text",
                            text: `[${block.mode === "parallel" ? "Parallel" : "Sequential"}] tool execution was skipped because a previous tool was rejected.`,
                        });
                        break;
                    }
                    if (this.didAlreadyUseTool) {
                        // Skip execution if a tool was already used
                        this.userMessageContent.push({
                            type: "text",
                            text: `[${block.mode === "parallel" ? "Parallel" : "Sequential"}] tool execution was not performed because a tool has already been used in this message.`,
                        });
                        break;
                    }
                    // Find all tools in the same group
                    const groupedTools = [];
                    // Add the current tool
                    groupedTools.push(block);
                    // Add other tools in the same group (those with the same mode)
                    for (let i = this.currentStreamingContentIndex + 1; i < this.assistantMessageContent.length; i++) {
                        const nextBlock = this.assistantMessageContent[i];
                        if (nextBlock.type === "tool_use" && nextBlock.mode === block.mode) {
                            groupedTools.push(nextBlock);
                        }
                        else {
                            break;
                        }
                    }
                    // Execute the tools based on the execution mode
                    const results = block.mode === "parallel"
                        ? await executeToolsInParallel(this, groupedTools)
                        : await executeToolsSequentially(this, groupedTools);
                    // Format and add the results to the user message
                    const formattedResults = formatGroupedToolResults(block.mode, results, groupedTools);
                    this.userMessageContent.push({
                        type: "text",
                        text: formattedResults,
                    });
                    // Mark that we've already used a tool
                    this.didAlreadyUseTool = true;
                    // Skip past all the grouped tools that we've already processed
                    this.currentStreamingContentIndex += groupedTools.length - 1;
                    break;
                }
                // Standard single tool execution (existing code)
                ...
                const askApproval = async (type, partialMessage, progressStatus) => {
                    ...
                    if (response !== "yesButtonClicked") {
                        ...
                        if (text) {
                            await this.say("user_feedback", text, images);
                            pushToolResult(formatResponse.toolResult(formatResponse.toolDenied() + `\n\nUser feedback: ${text}`, images));
                        }
                        else {
                    ...
                    if (text) {
                        await this.say("user_feedback", text, images);
                        pushToolResult(formatResponse.toolResult(`The user provided feedback: ${text}`, images));
                    }
                    return true;
                };
                ...
                switch (block.name) {
                    ...
                    case "insert_content":
                        ...
                        break;
                    case "search_and_replace":
                        await searchAndReplaceTool(this, block, askApproval, handleError, pushToolResult, removeClosingTag);
                        break;
                    case "read_file":
                    ...
                    case "list_files":
                        ...
                        break;
                    case "list_code_definition_names":
                        await listCodeDefinitionNamesTool(this, block, askApproval, handleError, pushToolResult, removeClosingTag);
                        break;
                    case "search_files":
                    ...
                    case "use_mcp_tool":
                        ...
                        break;
                    case "access_mcp_resource":
                        await accessMcpResourceTool(this, block, askApproval, handleError, pushToolResult, removeClosingTag);
                        break;
                    case "ask_followup_question":
                        await askFollowupQuestionTool(this, block, askApproval, handleError, pushToolResult, removeClosingTag);
                        break;
                    case "switch_mode":
    ...
    // Transform
    async parseUserContent(userContent) {
        ...
        return Promise.all(userContent.map(async (block) => {
            ...
            if (block.type === "text") {
                if (shouldProcessMentions(block.text)) {
                    return {
                        ...block,
                        text: await parseMentions(block.text, this.cwd, this.urlContentFetcher, this.fileContextTracker),
                    };
                }
                return block;
            }
                ...
                if (typeof block.content === "string") {
                    if (shouldProcessMentions(block.content)) {
                        return {
                            ...block,
                            content: await parseMentions(block.content, this.cwd, this.urlContentFetcher, this.fileContextTracker),
                        };
                    }
                    return block;
                }
                    ...
                    const parsedContent = await Promise.all(block.content.map(async (contentBlock) => {
                        if (contentBlock.type === "text" && shouldProcessMentions(contentBlock.text)) {
                            return {
                                ...contentBlock,
                                text: await parseMentions(contentBlock.text, this.cwd, this.urlContentFetcher, this.fileContextTracker),
                            };
                        }
                        return contentBlock;
                    }));
    ...
    getCheckpointService() {
        ...
        if (this.checkpointServiceInitializing) {
            console.log("[Klaus#getCheckpointService] checkpoint service is still initializing");
            return undefined;
        }
        const log = (message) => {
            console.log(message);
        ...
        console.log("[Klaus#getCheckpointService] initializing checkpoints service");
        try {
            const workspaceDir = getWorkspacePath();
            if (!workspaceDir) {
                log("[Klaus#getCheckpointService] workspace folder not found, disabling checkpoints");
                this.enableCheckpoints = false;
                return undefined;
            }
            const globalStorageDir = this.providerRef.deref()?.context.globalStorageUri.fsPath;
            if (!globalStorageDir) {
                log("[Klaus#getCheckpointService] globalStorageDir not found, disabling checkpoints");
                this.enableCheckpoints = false;
                return undefined;
            }
            const options = {
            ...
            service.on("initialize", () => {
                log("[Klaus#getCheckpointService] service initialized");
                try {
                    const isCheckpointNeeded = typeof this.clineMessages.find(({ say }) => say === "checkpoint_saved") === "undefined";
                    this.checkpointService = service;
                    this.checkpointServiceInitializing = false;
                    if (isCheckpointNeeded) {
                        log("[Klaus#getCheckpointService] no checkpoints found, saving initial checkpoint");
                        this.checkpointSave();
                    }
                }
                catch (err) {
                    log("[Klaus#getCheckpointService] caught error in on('initialize'), disabling checkpoints");
                    this.enableCheckpoints = false;
                }
            });
            service.on("checkpoint", ({ isFirst, fromHash: from, toHash: to }) => {
                try {
                    ...
                    this.say("checkpoint_saved", to, undefined, undefined, { isFirst, from, to }).catch((err) => {
                        log("[Klaus#getCheckpointService] caught unexpected error in say('checkpoint_saved')");
                        console.error(err);
                    });
                }
                catch (err) {
                    log("[Klaus#getCheckpointService] caught unexpected error in on('checkpoint'), disabling checkpoints");
                    console.error(err);
                    this.enableCheckpoints = false;
                }
            });
            log("[Klaus#getCheckpointService] initializing shadow git");
            service.initShadowGit().catch((err) => {
                log(`[Klaus#getCheckpointService] caught unexpected error in initShadowGit, disabling checkpoints (${err.message})`);
                console.error(err);
                this.enableCheckpoints = false;
            });
            return service;
            ...
            log("[Klaus#getCheckpointService] caught unexpected error, disabling checkpoints");
            this.enableCheckpoints = false;
            return undefined;
        }
    }
    async getInitializedCheckpointService({ interval = 250, timeout = 15_000, } = {}) {
        ...
        try {
            await pWaitFor(() => {
                console.log("[Klaus#getCheckpointService] waiting for service to initialize");
    ...
    async checkpointSave() {
        ...
        return service.saveCheckpoint(`Task: ${this.taskId}, Time: ${Date.now()}`).catch((err) => {
            console.error("[Klaus#checkpointSave] caught unexpected error, disabling checkpoints", err);
            this.enableCheckpoints = false;
        });
    }
    async checkpointRestore({ ts, commitHash, mode, }) {
        ...
        try {
            ...
            if (mode === "restore") {
                ...
                const { totalTokensIn, totalTokensOut, totalCacheWrites, totalCacheReads, totalCost } = getApiMetrics(combineApiRequests(combineCommandSequences(deletedMessages)));
                await this.overwriteKlausMessages(this.clineMessages.slice(0, index + 1));
            ...
            //
            // This was take from Klaus's implementation of the checkpoints
    ...
    /**
     * Executes a group of tools in parallel
     * @param toolCalls Array of tool calls to execute in parallel
     * @returns Dictionary of tool results by toolId
     */
    async executeToolsInParallel(toolCalls) {
        if (this.abort) {
            throw new Error(`[Klaus#executeToolsInParallel] task ${this.taskId}.${this.instanceId} aborted`);
        }
        const toolResults = new Map();
        // Create a description function for each tool
        const toolDescriptionFn = (block) => {
            switch (block.name) {
                case "execute_command":
                    return `[${block.name} for '${block.params.command}']`;
                case "read_file":
        ...
        const askApproval = async (block, type, partialMessage, progressStatus) => {
            const { response, text, images } = await this.ask(type, partialMessage, false, progressStatus);
            if (response !== "yesButtonClicked") {
                // Handle both messageResponse and noButtonClicked with text
                if (text) {
                    ...
                    return {
                        approved: false,
                        result: formatResponse.toolResult(formatResponse.toolDeniedWithFeedback(text), images)
                    };
                }
                else {
                    return {
            ...
            if (text) {
                ...
                return {
                    approved: true,
                    result: formatResponse.toolResult(formatResponse.toolApprovedWithFeedback(text), images)
                };
            }
            return { approved: true, result: undefined };
        };
        const removeClosingTag = (tag, text) => {
            if (!text) {
                return "";
        ...
        const handleError = async (block, action, error) => {
            const errorString = `Error ${action}: ${JSON.stringify(serializeError(error))}`;
            await this.say("error", `Error ${action}:\n${error.message ?? JSON.stringify(serializeError(error), null, 2)}`);
            return formatResponse.toolError(errorString);
        };
        // First validate all tools
        const { mode, customModes } = (await this.providerRef.deref()?.getState()) ?? {};
        for (const block of toolCalls) {
            try {
                ...
                this.consecutiveMistakeCount++;
                toolResults.set(block.toolId, formatResponse.toolError(error.message));
                // Return early with the existing results since validation failed
                return toolResults;
            }
        }
        // Execute each tool in parallel
        const toolPromises = toolCalls.map(async (block) => {
            if (this.abort) {
                return;
            }
            // Record tool usage
            this.recordToolUsage(block.name);
            telemetryService.captureToolUsage(this.taskId, block.name);
            // Close browser if not a browser action
            if (block.name !== "browser_action") {
                await this.browserSession.closeBrowser();
            }
            let toolResult;
            try {
                switch (block.name) {
                    case "write_to_file": {
                        // Create a function to handle the specific tool result
                        const pushResult = (result) => {
                            toolResult = result;
                        };
                        await writeToFileTool(this, block, async (type, partialMessage, progressStatus) => {
                            const result = await askApproval(block, type, partialMessage, progressStatus);
                            if (!result.approved) {
                            ...
                            toolResult = await handleError(block, action, error);
                        }, pushResult, removeClosingTag);
                        break;
                    }
                    case "apply_diff": {
                        const pushResult = (result) => {
                        ...
                        await applyDiffTool(this, block, async (type, partialMessage, progressStatus) => {
                            const result = await askApproval(block, type, partialMessage, progressStatus);
                            if (!result.approved) {
                                pushResult(result.result);
                                return false;
                    ...
                    // Add similar implementations for other tool types
                    // I'm showing a few examples but the pattern is the same for all tools
                    case "read_file": {
                        const pushResult = (result) => {
                            toolResult = result;
                        };
                        await readFileTool(this, block, async (type, partialMessage, progressStatus) => {
                            const result = await askApproval(block, type, partialMessage, progressStatus);
                            if (!result.approved) {
                                pushResult(result.result);
                                return false;
                    ...
                    case "search_files": {
                        ...
                        await searchFilesTool(this, block, async (type, partialMessage, progressStatus) => {
                            const result = await askApproval(block, type, partialMessage, progressStatus);
                            if (!result.approved) {
                                pushResult(result.result);
                                return false;
                    ...
                    // Additional case statements for other tools would follow the same pattern
                    // Omitting the rest for brevity, but they should be implemented in the same way
                }
                // Store result if it was defined
                if (toolResult && block.toolId) {
                    toolResults.set(block.toolId, toolResult);
                }
            }
            catch (error) {
                // Handle any unexpected errors
                toolResult = await handleError(block, "executing tool", error);
                if (block.toolId) {
                    toolResults.set(block.toolId, toolResult);
                }
            }
    ...
    /**
     * Executes a group of tools sequentially, respecting dependencies
     * @param toolCalls Array of tool calls to execute sequentially
     * @returns Dictionary of tool results by toolId
     */
    async executeToolsSequentially(toolCalls) {
        if (this.abort) {
            throw new Error(`[Klaus#executeToolsSequentially] task ${this.taskId}.${this.instanceId} aborted`);
        }
        const toolResults = new Map();
        ...
        while (pendingTools.length > 0) {
            ...
            for (let i = pendingTools.length - 1; i >= 0; i--) {
                ...
                if (!tool.dependsOn || processedToolIds.has(tool.dependsOn)) {
                    sortedTools.push(tool);
                    if (tool.toolId) {
                        processedToolIds.add(tool.toolId);
                    }
            ...
            if (initialLength === pendingTools.length && pendingTools.length > 0) {
                ...
                await this.say("error", "Warning: Circular dependency detected in sequential tool execution. Tools will be executed in their defined order.");
        ...
        for (const tool of sortedTools) {
            ...
            // Execute each tool and store its result
            const singleToolResults = await this.executeToolsInParallel([tool]);
            // Merge results
            for (const [toolId, result] of singleToolResults.entries()) {

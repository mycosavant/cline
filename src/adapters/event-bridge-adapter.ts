...
export class EventBridgeAdapter {
   ...
   */
  constructor(eventEmitter: EventEmitter, systemEventBus: any) {
  ...
  private setupEventBridges() {
    ...
    this.subscriptions.push(
      this.systemEventBus.subscribe('*', (event: any) => {
   ...
   */
  private setupSpecificEventMappings() {
    // File events
    ...
    // Memory events
    this.subscriptions.push(
      this.systemEventBus.subscribe('memory:entity-stored', (event: any) => {
        this.eventEmitter.emit('coding-memory:entity-stored', event.payload);
      })
    );
    
    this.subscriptions.push(
      this.systemEventBus.subscribe('memory:entity-updated', (event: any) => {
    ...
    // Cognitive events
    this.subscriptions.push(
      this.systemEventBus.subscribe('cognitive:insights-available', (event: any) => {
    ...
    this.subscriptions.push(
      this.systemEventBus.subscribe('tool:synthesized', (event: any) => {
    ...
    this.subscriptions.push(
      this.systemEventBus.subscribe('collaboration:mental-model-updated', (event: any) => {

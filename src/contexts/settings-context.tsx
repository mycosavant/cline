...
export interface AiSettings {
  ...
  geminiApiKey: string | null; // Allow storing user-provided key
  geminiModel: string; // Added Gemini model selection
  openRouterApiKey: string | null; // Allow storing user-provided key
  openRouterModel: string;
}

// Define structure for fetched models with more details
export interface ModelInfo {
    id: string;
    name: string;
    provider?: string; // Added provider field (e.g., 'openai', 'anthropic')
    description?: string; // Optional description
    contextLength: number | null; // Context window size
    inputCostMtok: number | null; // Cost per million input tokens (in USD)
    outputCostMtok: number | null; // Cost per million output tokens (in USD)
    // Add other relevant fields if needed, e.g., architecture, top_provider
}
...
interface SettingsContextValue {
  ...
  effectiveGeminiApiKey: string | null; // Key used by the app (env var or user input)
  effectiveOpenRouterApiKey: string | null; // Key used by the app (env var or user input)
  isInitialLoadComplete: boolean; // Flag indicating if settings have been loaded from storage
  // Add state for models (initially empty, fetched later)
  openRouterModels: ModelInfo[];
  fetchOpenRouterModels: () => Promise<void>; // Function to trigger fetch
  modelsLoading: boolean;
  modelsError: string | null;
  // Expose default settings via context
...
export function SettingsProvider({ children }: { children: React.ReactNode }) {
  ...
  const [settings, setSettings] = useState<AiSettings>(defaultSettings); // Initialize with defaults first

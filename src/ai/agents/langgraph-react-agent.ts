...
import { VscodeLM } from "@langchain/vscode";
...
const installNodePackages = tool(
  async ({ command }) => {
    try {
...
const documentCodebaseTool = tool(
  async () => {
...
// Define the tools
const tools: Tool[] = [installNodePackages, documentCodebaseTool];

...
export class RelationshipGraphPanel {
    ...
    static create(mcpManager) {
        const panel = vscode.window.createWebviewPanel('relationshipGraph', 'Code Relationship Graph', vscode.ViewColumn.Two, {
            enableScripts: true,
    ...
    async update() {
        ...
        try {
            ...
            const entityGraphAction = memoryAdapter.actions.find(action => action.name === 'getEntityGraph');
    ...
    getMermaidHtml(mermaidDefinition) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
...
//# sourceMappingURL=relationship-graph-panel.js.map
 ...
 * ModeConfig
 */
const groupEntryArraySchema = z.array(groupEntrySchema).refine((groups) => {
    const seen = new Set();
 ...
 * ExperimentId
 */
export const experimentIds = ["autoCondenseContext", "powerSteering"];
...
export const GLOBAL_STATE_KEYS = [...GLOBAL_SETTINGS_KEYS, ...PROVIDER_SETTINGS_KEYS].filter((key) => !SECRET_STATE_KEYS.includes(key));
 ...
 * ToolUsage
 */
export const toolUsageSchema = z.record(toolNamesSchema, z.object({
    attempts: z.number(),
    failures: z.number(),
}));
/**

...
import { useSettings, type ModelInfo, type AiSettings } from '@/contexts/settings-context'; // Import ModelInfo and AiSettings
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { Info, KeyRound, Loader2, AlertTriangle, Search, ArrowUpDown, DollarSign, Layers, Blocks, Maximize } from 'lucide-react'; // Added Blocks, Maximize
...
type SortCriteria = 'cost' | 'context' | 'name';
...
const formatContext = (length: number | null | undefined): string => {
    ...
    if (length >= 1000000) return `${(length / 1000000).toLocaleString()}M`;
    if (length >= 1000) return `${(length / 1000).toLocaleString()}K`;
    return length.toLocaleString();
};

// Component to display model details
const ModelDetailsCard: React.FC<{ model: ModelInfo | null }> = ({ model }) => {
    if (!model) {
        return (
...
export function AiSettingsDialog() {
  const {
      ...
      defaultSettings: contextDefaultSettings // Get default settings from context
  } = useSettings();
  ...
  const [sortCriteria, setSortCriteria] = useState<SortCriteria>('cost');
  const [selectedModelDetails, setSelectedModelDetails] = useState<ModelInfo | null>(null); // State for selected model details
        ...
        <TooltipProvider>
            ...
            <div className="grid grid-cols-4 items-center gap-x-4 gap-y-1">
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                      const newProvider = value as 'gemini' | 'openrouter';
            ...
            {localSettings.provider === 'gemini' && (
                <Card className="border-border/50 bg-background mt-4"> {/* Add margin top */}
                    <CardHeader className="pb-2 pt-4">
                    ...
                    <CardContent className="pt-0 grid gap-4"> {/* Remove top padding */}
                        ...
                        <div className="grid grid-cols-4 items-center gap-x-4 gap-y-1">
                            ...
                            <Input
                                id="gemini-api-key"
                                type="password"
                                placeholder={!isClient ? "Loading..." : isGeminiKeySetServerSide ? "Using server-side key" : "Enter Gemini API Key"}
                                value={localSettings.geminiApiKey || ''}
                                onChange={(e) =>
                                setLocalSettings({ ...localSettings, geminiApiKey: e.target.value || null })
                                }
                                className="col-span-3 h-9" // Smaller height
                        ...
                        <div className="grid grid-cols-4 items-center gap-x-4 gap-y-1">
                            ...
                            <Select
                                ...
                                onValueChange={(value) =>
                                    setLocalSettings({ ...localSettings, geminiModel: value })
                                }
                                disabled={!isClient}
                            >
                    ...
                    <CardContent className="pt-0 grid gap-4"> {/* Remove top padding */}
                        ...
                        <div className="grid grid-cols-4 items-center gap-x-4 gap-y-1">
                            ...
                            <Input
                              ...
                              type="password"
                              placeholder={!isClient ? "Loading..." : isOpenRouterKeySetServerSide ? "Using server-side key" : "Enter OpenRouter API Key"}
                              value={localSettings.openRouterApiKey || ''}
                              onChange={(e) =>
                                  setLocalSettings({ ...localSettings, openRouterApiKey: e.target.value || null })
                              }
                              className="col-span-3 h-9"
                              disabled={!isClient || isOpenRouterKeySetServerSide}
                            />
                        </div>
                         {/* Model Selection */}
                        <div className="grid grid-cols-4 items-center gap-x-4 gap-y-1">
                            ...
                            <div className="col-span-3 flex flex-col">
                                <Select
                                     ...
                                     onValueChange={(value) =>
                                         setLocalSettings({ ...localSettings, openRouterModel: value })
                                     }
                                     disabled={!isClient || modelsLoading || !!modelsError || openRouterModels.length === 0}
                                 >
                                     ...
                                     <SelectContent>
                                         ...
                                         <div className="p-2 sticky top-0 bg-popover z-10 shadow-sm">
                                             <div className="relative mb-2">
                                                 ...
                                                 <Input
                                                     ...
                                                     className="pl-8 h-8 text-xs" // Smaller search input
                                         ...
                                         {modelsLoading ? (
                                             ...
                                             <div className="p-4 text-center text-destructive text-sm">{modelsError}</div>
                                             ...
                                             <ScrollArea className="h-[200px]"> {/* Adjust height as needed */}
                                              ...
                                              <div className="p-4 text-center text-muted-foreground text-sm">No models match '{searchTerm}'.</div>
                                         )}
                                     </SelectContent>
                                 </Select>

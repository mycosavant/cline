...
export default function ChatInterface() {
  ...
  const {
    ...
    input,
    handleInputChange,
    handleSubmit: handleChatSubmit, // Rename to avoid conflict
    setInput,
    isLoading,
    setMessages,
    reload, // Add reload function
  ...
  const isProviderConfigured = useMemo(() => {
    ...
    if (!isInitialLoadComplete || !isClient) return false;
  ...
  const handleCopy = (content: string, type: 'message' | 'code') => {
  ...
  const handleSubmitWrapper = (e?: React.FormEvent<HTMLFormElement>) => {
     ...
     if (!isProviderConfigured) {
         ...
         toast({
             ...
             variant: "destructive",
             action: (
                 <Button variant="link" onClick={() => setIsSettingsOpen(true)}>
                     Open Settings
                 </Button>
             ),
         });
  ...
  const handleResubmit = (messageId: string) => {
    const messageToResubmit = messages.find((msg) => msg.id === messageId);
    if (!messageToResubmit || messageToResubmit.role !== 'user') return;
    ...
    const editedUserMessage: Message = { ...messageToResubmit, content: editContent };
    ...
    append(
        { role: 'user', content: editContent }, // Provide the content to append
    ...
    <Markdown
      ...
      components={{
        code({ node, inline, className, children, ...props }) {
          ...
          return !inline ? (
            <div className="relative my-2 rounded-md bg-gray-900 font-mono text-sm text-gray-200 shadow-inner backdrop-blur-sm">
            ...
            <code className="rounded bg-muted/50 dark:bg-muted/20 px-1 py-0.5 font-mono text-sm" {...props}>
              {children}
            </code>
          );
        },
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden bg-gradient-to-br from-background via-background/80 to-background/60 backdrop-blur-xl dark:from-card dark:via-card/80 dark:to-card/60">
      {/* Configuration Alert */}
      {isClient && isInitialLoadComplete && !isProviderConfigured && (
        <Alert variant="destructive" className="m-4 rounded-lg border-destructive/30 bg-destructive/10 backdrop-blur-sm">
       ...
       {(contextContent || selectedFile) && (
           <div className="mx-4 mb-2 p-3 rounded-lg border border-border/50 bg-muted/30 backdrop-blur-sm text-xs text-muted-foreground shadow-sm">
        ...
        <div className="space-y-6 pb-4 group/chat-area"> {/* Added group */}
          {messages.map((message, index) => (
            <div
              key={message.id}
              className={cn(
                'flex items-start gap-3 group/message', // Added group/message
            ...
            >
              ...
              <Card
                className={cn(
                  'max-w-[85%] rounded-xl p-3 shadow-md relative backdrop-blur-md border border-border/20',
                   'transition-all duration-300 ease-out', // Added transition
                  message.role === 'user'
                ...
                {editingMessageId === message.id ? (
                  ...
                  <div className="space-y-2">
                    <Textarea
                      ...
                      className="min-h-[60px] text-sm bg-background/80 dark:bg-black/30 border-border/50 focus:ring-accent"
                      rows={3}
                    />
                    <div className="flex justify-end gap-2">
                      ...
                      <Button
                        ...
                        disabled={!editContent.trim() || isLoading} // Disable if loading
                  ...
                  <>
                    <CardContent className="p-0 text-sm break-words">
                      <MarkdownRenderer content={message.content} />
                       ...
                       {message.role === 'assistant' && message.data?.artifact && (
                      ...
                      <div className="absolute -top-3 -right-2 flex items-center gap-0.5 opacity-0 group-hover/message:opacity-100 transition-opacity duration-200">
                         {message.role === 'user' && !isLoading && ( // Only show edit for user, not while loading
                             <Button
                                variant="ghost"
                                size="icon"
                                className="h-6 w-6 text-muted-foreground hover:text-foreground bg-background/70 dark:bg-card/70 backdrop-blur-sm rounded-full shadow"
                         ...
                         <Button
                          ...
                          {message.role === 'user' && index === messages.length - 2 && error && !isLoading && (
                                <Button
                                    variant="ghost"
                                    size="icon"
                                    className="h-6 w-6 text-red-500 hover:text-red-700 bg-background/70 dark:bg-card/70 backdrop-blur-sm rounded-full shadow"
                                    onClick={() => reload()} // Reloads the last exchange
                                    title="Retry request"
      ...
      {/* Input Area */}
       <div className="border-t border-border/20 bg-background/60 dark:bg-card/60 p-4 shadow-inner backdrop-blur-xl">
         ...
         <div className="mb-3 flex flex-wrap gap-2">
             {samplePrompts.map((prompt, i) => (
                 <Button
                     ...
                     variant="outline"
                     size="sm"
                     className="rounded-full text-xs h-7 bg-muted/30 backdrop-blur-sm border-border/30 hover:bg-muted/60 hover:border-border/50 transition-all duration-200"
        ...
        <form
          ...
          className="relative flex items-end gap-2" // items-end aligns button with bottom of taller textarea
        >
          <Textarea
            ref={inputRef}
            value={input}
            ...
            className={cn(
               'flex-1 resize-none rounded-lg border border-input bg-background/80 dark:bg-muted/50 backdrop-blur-sm px-4 py-2.5 pr-12 text-sm shadow-sm focus:border-accent focus:ring-accent/50 dark:text-foreground',
               'min-h-[52px]' // Ensure minimum height
            )}
            rows={1} // Start with one row, auto-grows
            disabled={isInputDisabled}
            style={{ maxHeight: '200px' }} // Limit max height
            aria-label="Chat input"
          />
          <Button
            type="submit"
            size="icon"
            className="absolute right-[10px] bottom-[10px] h-9 w-9 rounded-lg bg-accent text-accent-foreground shadow-md hover:bg-accent/90 disabled:opacity-50 transition-all"
          ...
          >
            {isLoading ? (
              ...
              <ArrowUp className="h-4 w-4" /> // Changed to ArrowUp for modern look
            )}
          </Button>
        </form>
      </div>

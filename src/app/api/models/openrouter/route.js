...
export async function GET(request) {
    ...
    try {
        ...
        }).filter((model) => !!model.id); // Ensure model has an ID and filter out potential nulls/undefined
        ...
        return NextResponse.json({ error: `Failed to fetch OpenRouter models: ${error.message || 'Unknown error'}` }, { status: 500 });
    }
}

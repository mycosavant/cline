...
export class OpenAiHandler extends BaseProvider {
    ...
    async *createMessage(systemPrompt, messages) {
        ...
        if (this.options.openAiStreamingEnabled ?? true) {
            ...
            const stream = await this.client.chat.completions.create(requestOptions, isAzureAiInference ? { path: AZURE_AI_INFERENCE_PATH } : {});
            ...
            const response = await this.client.chat.completions.create(requestOptions, this._isAzureAiInference(modelUrl) ? { path: AZURE_AI_INFERENCE_PATH } : {});
            yield {
                type: "text",
    ...
    async completePrompt(prompt) {
        try {
            ...
            const response = await this.client.chat.completions.create(requestOptions, isAzureAiInference ? { path: AZURE_AI_INFERENCE_PATH } : {});
    ...
    async *handleO3FamilyMessage(modelId, systemPrompt, messages) {
        if (this.options.openAiStreamingEnabled ?? true) {
            ...
            const methodIsAzureAiInference = this._isAzureAiInference(this.options.openAiBaseUrl);
            const response = await this.client.chat.completions.create(requestOptions, methodIsAzureAiInference ? { path: AZURE_AI_INFERENCE_PATH } : {});
            yield {
                type: "text",
                text: response.choices[0]?.message.content || "",
            };

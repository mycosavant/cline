...
describe("Cost Utilities", () => {
  describe("calculateApiCostAnthropic", () => {
    ...
    it("should use real model configuration (Claude 3 Haiku)", async () => {
      const pricing = JSON.parse(await fs.readFile('data/pricing.json', 'utf-8')) as { models: { name: string; input_price: number; output_price: number; }[] };
      const haikuPricing = pricing.models.find((m) => m.name === "claude-3-haiku");
      if (!haikuPricing) {
        throw new Error("Claude 3 Haiku pricing not found");
      }
      const modelInfo: ModelInfo = {
        maxTokens: 8192,
        contextWindow: 200_000,
        ...
        supportsComputerUse: true,
        supportsPromptCache: true,
        inputPrice: haikuPricing.input_price * 1000,
        outputPrice: haikuPricing.output_price * 1000,
      };
      const cost = calculateApiCostAnthropic(modelInfo, 1000, 1000);
      // Input: (0.00025 / 1000) * 1000 = 0.00025
      // Output: (0.000125 / 1000) * 1000 = 0.000125
      // Total: 0.00025 + 0.000125 = 0.000375
      cost.should.equal(0.000375);
    });
  });
